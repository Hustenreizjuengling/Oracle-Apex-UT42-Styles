--------------------------------------------------------------------------------
-- Frequenz 1.0.0 – Deinstallation der Theme-Variante
-- Weiße Seite, hellgraue Tafeln mit großen Radien und ein Farbblock in der Akzentfarbe oben links, der den App-Namen trägt. Kräftige Überschriften in Onest (800), Kontur-Buttons mit 4 px Radius, runde Icon-Buttons und Pillen, eine 4-px-Kante in der Akzentfarbe am aktiven Navigationspunkt, Links in Petrol. Dunkel: schwarze Seite mit dunkelgrauen Tafeln. Für interne Apps.
--
...
--
-- Aufruf:
--   @frequenz-theme-uninstall.sql <APP_ID>
--
-- Entfernt das Theme 144 "Frequenz" samt Theme-Dateien und Styles
-- (wwv_flow_imp_shared.delete_theme). Das Universal Theme der App bleibt unberührt.
-- Ist Frequenz das aktuelle Theme der App (-20006) oder wurde es entkoppelt und die App
...
-- Rückweg per Switch Theme (Frequenz -> Universal Theme) scheitert in APEX 26.1 an den
-- Template Components.
-- Auch eigene Styles am Theme 144 (z. B. aus dem Theme Roller) werden entfernt.
--------------------------------------------------------------------------------
prompt === Frequenz 1.0.0: Deinstallation des Themes 144 ===
...
prompt ... Theme 144 entfernen
declare
  c_theme  constant number := 144;
  l_app_id number;
  l_ws     number;
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @frequenz-theme-uninstall.sql 100');
  end if;
  ...
  if not (upper(l_theme.theme_internal_name) = 'FREQUENZ' or lower(l_theme.static_id) = 'frequenz') then
    raise_application_error(-20003, 'Theme ' || c_theme || ' ("' || l_theme.theme_name || '") in Application ' || l_app_id || ' ist nicht Frequenz – es wird nicht entfernt.');
  end if;
  if l_theme.is_current = 'Yes' then
    raise_application_error(-20006, 'Frequenz ist das aktuelle Theme von Application ' || l_app_id || '. Bitte zuerst die App aus dem Backup (Export vor dem Unsubscribe) wiederherstellen, dann erneut ausführen.');
  end if;
  ...
  for s in (select name
             ...
             where application_id = l_app_id and theme_number = c_theme
               and is_subscribed = 'No' and name not in ('Frequenz Light', 'Frequenz Dark', 'Frequenz Auto'))
  loop

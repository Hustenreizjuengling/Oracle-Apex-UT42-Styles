--------------------------------------------------------------------------------
-- Orbit 1.0.0 – Deinstallation der Theme-Variante
...
--
-- Aufruf:
--   @orbit-theme-uninstall.sql <APP_ID>
--
-- Entfernt das Theme 147 "Orbit" samt Theme-Dateien und Styles
-- (wwv_flow_imp_shared.delete_theme). Das Universal Theme der App bleibt unberührt.
-- Ist Orbit das aktuelle Theme der App (-20006) oder wurde es entkoppelt und die App
...
-- Rückweg per Switch Theme (Orbit -> Universal Theme) scheitert in APEX 26.1 an den
-- Template Components.
-- Auch eigene Styles am Theme 147 (z. B. aus dem Theme Roller) werden entfernt.
--------------------------------------------------------------------------------
prompt === Orbit 1.0.0: Deinstallation des Themes 147 ===
...
prompt ... Theme 147 entfernen
declare
  c_theme  constant number := 147;
  l_app_id number;
  l_ws     number;
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @orbit-theme-uninstall.sql 100');
  end if;
  ...
  if not (upper(l_theme.theme_internal_name) = 'ORBIT' or lower(l_theme.static_id) = 'orbit') then
    raise_application_error(-20003, 'Theme ' || c_theme || ' ("' || l_theme.theme_name || '") in Application ' || l_app_id || ' ist nicht Orbit – es wird nicht entfernt.');
  end if;
  if l_theme.is_current = 'Yes' then
    raise_application_error(-20006, 'Orbit ist das aktuelle Theme von Application ' || l_app_id || '. Bitte zuerst die App aus dem Backup (Export vor dem Unsubscribe) wiederherstellen, dann erneut ausführen.');
  end if;
  ...
  for s in (select name
             ...
             where application_id = l_app_id and theme_number = c_theme
               and is_subscribed = 'No' and name not in ('Orbit Light', 'Orbit Dark', 'Orbit Auto'))
  loop

--------------------------------------------------------------------------------
-- Veedel 1.0.0 – Deinstallation der Theme-Variante
-- Veedel – Theme Styles für das Universal Theme 42: Kopfband in tiefem Markenblau, weiße Navigation mit blauem Aktiv-Block, Seitentitel als Verlaufsband (Tiefblau → Leuchtazur) mit großer weißer Headline, weiße eckige Flächen ohne Schatten auf hellgrauem Grund, Signalrot nur für die Hauptaktion, Gefahr als Karmin-Kontur. Dark in Nachtblau, Auto folgt dem Betriebssystem. Schrift: Figtree (OFL).
--
...
--
-- Aufruf:
--   @veedel-theme-uninstall.sql <APP_ID>
--
-- Entfernt das Theme 143 "Veedel" samt Theme-Dateien und Styles
-- (wwv_flow_imp_shared.delete_theme). Das Universal Theme der App bleibt unberührt.
-- Ist Veedel das aktuelle Theme der App (-20006) oder wurde es entkoppelt und die App
...
-- Rückweg per Switch Theme (Veedel -> Universal Theme) scheitert in APEX 26.1 an den
-- Template Components.
-- Auch eigene Styles am Theme 143 (z. B. aus dem Theme Roller) werden entfernt.
--------------------------------------------------------------------------------
prompt === Veedel 1.0.0: Deinstallation des Themes 143 ===
...
prompt ... Theme 143 entfernen
declare
  c_theme  constant number := 143;
  l_app_id number;
  l_ws     number;
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @veedel-theme-uninstall.sql 100');
  end if;
  ...
  if not (upper(l_theme.theme_internal_name) = 'VEEDEL' or lower(l_theme.static_id) = 'veedel') then
    raise_application_error(-20003, 'Theme ' || c_theme || ' ("' || l_theme.theme_name || '") in Application ' || l_app_id || ' ist nicht Veedel – es wird nicht entfernt.');
  end if;
  if l_theme.is_current = 'Yes' then
    raise_application_error(-20006, 'Veedel ist das aktuelle Theme von Application ' || l_app_id || '. Bitte zuerst die App aus dem Backup (Export vor dem Unsubscribe) wiederherstellen, dann erneut ausführen.');
  end if;
  ...
  for s in (select name
             ...
             where application_id = l_app_id and theme_number = c_theme
               and is_subscribed = 'No' and name not in ('Veedel Light', 'Veedel Dark', 'Veedel Auto'))
  loop

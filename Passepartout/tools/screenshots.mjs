// Erzeugt die Vorschaubilder für die README (Passepartout/screenshots/).
//   node Passepartout/tools/screenshots.mjs
// Voraussetzung: Testbett mit aktivem Labor-Style (siehe README im Projektordner).
// Zwischenbilder: _tmp/<prefix>/readme-shots (prefix aus theme.json – eigener Ordner je Theme, wird
// bei jedem Lauf geleert; parallel laufende Themes kommen sich so nicht in die Quere).
...
const OUT = path.join(ROOT, 'Passepartout', 'screenshots');
const TMP = path.join(ROOT, '_tmp', meta.prefix, 'readme-shots');
...
async function run(style, { mobile = false } = {}) {
  const lab = await launchLab({ theme: 'Passepartout', style, app: 9042, mobile, width: mobile ? 390 : 1440, height: mobile ? 844 : 900 });
  try {

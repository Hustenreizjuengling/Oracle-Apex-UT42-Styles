// Erzeugt die Vorschaubilder für die README (Kracherl/screenshots/).
//   node Kracherl/tools/screenshots.mjs
...
const OUT = path.join(ROOT, 'Kracherl', 'screenshots');
const TMP = path.join(ROOT, '_tmp', 'kracherl', 'readme-shots');   // eigener Arbeitsordner (Parallelbetrieb)
...
async function run(style, { mobile = false } = {}) {
  const lab = await launchLab({ theme: 'Kracherl', style, app: 9042, mobile, width: mobile ? 390 : 1440, height: mobile ? 844 : 900 });
  try {
    for (const [name, pageId, prep] of mobile ? [['mobil', 1101], ['mobil-navigation', 1101, async (p) => p.click('#t_Button_navControl')], ['mobil-login', 9999]] : SHOTS) {
...
sheet(
  ...
  ['mobil-light', 'mobil-navigation-light', 'mobil-login-light', 'mobil-dark', 'mobil-navigation-dark', 'mobil-login-dark'].map((n) => path.join(TMP, `${n}.png`)),
  6,
  2400,
);

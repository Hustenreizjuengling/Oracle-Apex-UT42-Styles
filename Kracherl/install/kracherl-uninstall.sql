...
--
-- Aufruf:
--   @kracherl-uninstall.sql <APP_ID>
--
-- Was passiert:
--   1. Ist ein Kracherl-Style aktiv, wird auf "Vita" zurückgeschaltet.
--   2. Die Kracherl-Styles werden deaktiviert (nicht mehr öffentlich, nur noch Vita-CSS).
--   3. Alle statischen Dateien unter #APP_FILES#kracherl/ werden gelöscht.
--
...
--------------------------------------------------------------------------------
prompt === Kracherl 1.0.0: Deinstallation ===
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @kracherl-install.sql 100 light');
  end if;
...
begin
  select max(current_theme_style) into l_current
    from apex_application_themes
   where application_id = l_app_id and theme_number = 42;
  if l_current in ('Kracherl Light', 'Kracherl Dark', 'Kracherl Auto') then
...
begin
  select count(*) into l_cnt from apex_application_theme_styles
   where application_id = wwv_flow_application_install.get_application_id and theme_number = 42 and name = 'Kracherl Light';
  if l_cnt > 0 then
    upsert_style(p_name => 'Kracherl Light', p_static_id => 'kracherl-light',
                 p_css => '#THEME_FILES#css/Vita#MIN#.css?v=#APEX_VERSION#', p_public => false);
  end if;
  select count(*) into l_cnt from apex_application_theme_styles
   where application_id = wwv_flow_application_install.get_application_id and theme_number = 42 and name = 'Kracherl Dark';
  if l_cnt > 0 then
    upsert_style(p_name => 'Kracherl Dark', p_static_id => 'kracherl-dark',
                 p_css => '#THEME_FILES#css/Vita#MIN#.css?v=#APEX_VERSION#', p_public => false);
  end if;
  select count(*) into l_cnt from apex_application_theme_styles
   where application_id = wwv_flow_application_install.get_application_id and theme_number = 42 and name = 'Kracherl Auto';
  if l_cnt > 0 then
    upsert_style(p_name => 'Kracherl Auto', p_static_id => 'kracherl-auto',
                 p_css => '#THEME_FILES#css/Vita#MIN#.css?v=#APEX_VERSION#', p_public => false);
  end if;
end;
/

--------------------------------------------------------------------------------
-- Kracherl 1.0.0 – Deinstallation der Theme-Variante
-- Kracherl – sonniges Biergarten-Theme: sonnengelbes Kopfband mit einer einzelnen, einfarbigen Wellenkante und Sprudelblasen, Überschriften in Etikett-Violett (Bitter), Text in PT Sans, weiße Etiketten-Karten auf Lavendel-Grund, der aktuelle Eintrag als gelbe Etikett-Pille, Pillen-Buttons, Brause-Orange für Fokus und „hier bin ich“. Dunkel: „Biergarten bei Nacht“ – Violett-Schwarz mit Laternengelb.
--
...
--
-- Aufruf:
--   @kracherl-theme-uninstall.sql <APP_ID>
--
-- Entfernt das Theme 145 "Kracherl" samt Theme-Dateien und Styles
-- (wwv_flow_imp_shared.delete_theme). Das Universal Theme der App bleibt unberührt.
-- Ist Kracherl das aktuelle Theme der App (-20006) oder wurde es entkoppelt und die App
...
-- Rückweg per Switch Theme (Kracherl -> Universal Theme) scheitert in APEX 26.1 an den
-- Template Components.
-- Auch eigene Styles am Theme 145 (z. B. aus dem Theme Roller) werden entfernt.
--------------------------------------------------------------------------------
prompt === Kracherl 1.0.0: Deinstallation des Themes 145 ===
...
prompt ... Theme 145 entfernen
declare
  c_theme  constant number := 145;
  l_app_id number;
  l_ws     number;
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @kracherl-theme-uninstall.sql 100');
  end if;
  ...
  if not (upper(l_theme.theme_internal_name) = 'KRACHERL' or lower(l_theme.static_id) = 'kracherl') then
    raise_application_error(-20003, 'Theme ' || c_theme || ' ("' || l_theme.theme_name || '") in Application ' || l_app_id || ' ist nicht Kracherl – es wird nicht entfernt.');
  end if;
  if l_theme.is_current = 'Yes' then
    raise_application_error(-20006, 'Kracherl ist das aktuelle Theme von Application ' || l_app_id || '. Bitte zuerst die App aus dem Backup (Export vor dem Unsubscribe) wiederherstellen, dann erneut ausführen.');
  end if;
  ...
  for s in (select name
             ...
             where application_id = l_app_id and theme_number = c_theme
               and is_subscribed = 'No' and name not in ('Kracherl Light', 'Kracherl Dark', 'Kracherl Auto'))
  loop

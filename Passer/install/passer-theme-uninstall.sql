--------------------------------------------------------------------------------
-- Passer 1.0.0 – Deinstallation der Theme-Variante
-- Zweifarbendruck auf kühlem Papier: Teal als Arbeitsfarbe, Fluoreszenz-Pink als zweiter, leicht verrutschter Farbauszug (Passerversatz an Seitentitel, Logo, Hot-Buttons, Volltonbadges, aktiver Navigation und aktivem Reiter – nie auf Fließtext, Feldern oder Tabellen), Text in Indigo-Tusche, Gelb als Marker. Papierkorn, Halbton-Raster, Login als Plakat mit Überdruck-Scheiben. Nachts Neon auf indigo gefärbtem Papier. Schrift: Bricolage Grotesque (variabel, opsz 12–96, Gewicht 200–800, Breite 75–100 %). Vertrag für Komponenten: docs/ARCHITECTURE.md.
--
...
--
-- Aufruf:
--   @passer-theme-uninstall.sql <APP_ID>
--
-- Entfernt das Theme 146 "Passer" samt Theme-Dateien und Styles
-- (wwv_flow_imp_shared.delete_theme). Das Universal Theme der App bleibt unberührt.
-- Ist Passer das aktuelle Theme der App (-20006) oder wurde es entkoppelt und die App
...
-- Rückweg per Switch Theme (Passer -> Universal Theme) scheitert in APEX 26.1 an den
-- Template Components.
-- Auch eigene Styles am Theme 146 (z. B. aus dem Theme Roller) werden entfernt.
--------------------------------------------------------------------------------
prompt === Passer 1.0.0: Deinstallation des Themes 146 ===
...
begin
  if '&APP_ID.' is null then
    raise_application_error(-20000, 'Bitte die Application ID als ersten Parameter angeben, z. B.: @passer-theme-uninstall.sql 100');
  end if;
  ...
  if not (upper(l_theme.theme_internal_name) = 'PASSER' or lower(l_theme.static_id) = 'passer') then
    raise_application_error(-20003, 'Theme ' || c_theme || ' ("' || l_theme.theme_name || '") in Application ' || l_app_id || ' ist nicht Passer – es wird nicht entfernt.');
  end if;
  if l_theme.is_current = 'Yes' then
    raise_application_error(-20006, 'Passer ist das aktuelle Theme von Application ' || l_app_id || '. Bitte zuerst die App aus dem Backup (Export vor dem Unsubscribe) wiederherstellen, dann erneut ausführen.');
  end if;
  ...
  for s in (select name
             ...
             where application_id = l_app_id and theme_number = c_theme
               and is_subscribed = 'No' and name not in ('Passer Light', 'Passer Dark', 'Passer Auto'))
  loop
